...
downloads. JobClaw only provides an async boundary and maps results into stable
...
from jobclaw.auth.cookie_manager import get_cookies
...
class SpiderXhsBackend:
    """Use Spider_XHS directly inside the JobClaw process."""
    ...
    async def start(self) -> None:
        ...
        if missing:
            ...
            raise XhsAuthenticationError(
                ...
                "Provide XHS_COOKIE_HEADER or run `jobclaw login --platform xhs`."
            )
    ...
    async def download_note(
        ...
        *,
        ...
        async with self._request_lock:
            return await asyncio.to_thread(self._download_note_sync, url, destination)
    ...
    def _search_notes_sync(
        ...
        success, message, items = self._api.search_some_note(
            query=query,
            require_num=limit,
    ...
    def _fetch_note_sync(self, url: str) -> XhsFetchedNote:
        ...
        success, message, raw_response = self._api.get_note_info(url)
        if not success:
    ...
    def _download_note_sync(self, url: str, destination: Path) -> DownloadedXhsNote:
        bindings = cast(SpiderXhsBindings, self._bindings)
        note = self._fetch_note_sync(url)
        saved = bindings.download_note(note.normalized, str(destination), "media-image")
        directory = Path(saved).resolve()
