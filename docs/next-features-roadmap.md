...
| TR-4b | 打招呼后自动发送对应附件简历：聊天窗口「发简历」按钮流程。✅ 按钮 DOM 已到手（2026-08-28 用户贴图：`.chat-controls` 工具栏 `.toolbar-btn` 文本「 发简历 」，chat-new v5535） | 高（用户 2026-08-27 定） | HR 收到 PDF；每个岗位发送的简历版本入册；发送后确认信号待挖 |
| TR-5a | ✅ 离线交付（2026-08-28，待真机验证）：`list_boss_greetings` 工具已实现——聊天页页内 fetch 走 `geekFilterByLabel`，返回 HR 名/公司/职位/最后消息/来源，过滤系统账号（friendId<=1000）。读操作无 HITL 门 | 已上线 | 真机验证点：① 聊天页导航是否被 warlock 拦 ② API 返回结构 ③ labelId↔8 tab 映射（先只校准「全部」=0，其余 tab 的 id 在 UI chunk 里，用返回的原始数据逐个试） |
| TR-5b | 过滤器 tab 完整映射：未读/新招呼/仅沟通/有交换/有面试/不感兴趣 ↔ labelId | 中 | 真机校准（可在聊天页 network 里看 App 切 tab 时的请求） | API 已挖到（geek-chat-core.2.0.3.umd.min.js，静态分析）：`GET /wapi/zprelation/friend/geekFilterByLabel?labelId=<n>` 按标签过滤 + `POST /wapi/zprelation/friend/getGeekFriendList.json` 分页列表 + `/wapi/zpmsg/history/pull`、`/wapi/zpchat/boss/historyMsg` 历史消息。labelId 与 8 个过滤 tab 的映射需真机校准（已知 labelId:-1 与 0 的用法） |
| TR-6a | ✅ 离线交付（2026-08-28，待真机验证）：`reply_boss_greeting` 工具已实现——聊天页 UI 锚点探测链（搜索框→列表项→输入区→发送按钮，每步多形态探测+结构化失败），人类节奏打字（30ms/字），输入框清空=发送成功判定。HITL：user_confirmed 必填 | 已上线 | 真机验证点：① 锚点选择器命中 ② 发送后输入框清空判定 ③ UI 交互是否触发 warlock（用户手动聊天不触发，自动化复刻理论安全，未验证）。⚠️ 2026-08-28 实测：聊天页连续加载 3 次即被 warlock 拦——需要熔断器（F-见下）配合 |
| TR-6a.1 | ✅ 持久聊天页（2026-08-28）：`boss_chat_session.get_chat_page()`——聊天页每进程只加载一次后停靠（detached from tab pool），list/reply 共享；列表刷新=纯 fetch 不重载页面；页面留在 Chrome 里供用户接着人工用。教训：快速 开→扫→关 三连第 3 次即被 warlock 拦 | 已上线 | 跨进程（每次 `chat -c` 是新进程）仍需一次加载——节奏由使用者控制，避免连续快速调用 |
| TR-6b | ✅ 已交付（2026-08-28）：Boss 熔断器 `BossCircuit`——连续 3 次 page_lost/page_blocked/tab_pool_timeout → 熔断 4 小时，期间全部 Boss 工具（greet/upload/list/history/reply）入口直接返回 `circuit_open`；成功调用归零计数；状态落 `data/boss_circuit.json` 跨进程共享 | 已上线 | 触发错误集可按需扩充（如 security_id_unavailable 是否算前兆） |⚠️ 消息发送不走 REST——走 WebSocket（`/wapi/zpchat/config/ws` 拿配置 + SharedWorker RPC，协议在 geek-chat-core）。首选路线：页内驱动聊天页已加载的 chat-core 实例发消息（Boss 自己的代码在发，指纹完全一致）；备选：聊天页 UI 自动化（风险未知，chat 页点击未实测） | 高（依赖 TR-5 先通） | 频控严格：回复间隔 + 每日上限；内容审核（Boss 对消息内容有风控）；失败如实报告 |
