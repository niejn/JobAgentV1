...
**HR Conversation（HR 会话）**:
求职者与某位招聘者围绕一个或多个岗位形成的外部平台沟通通道；它可能在岗位卡片的“立即沟通”动作后才创建，
并独立于单条消息、岗位旅程阶段和 Agent 对话。
_Avoid_: Agent 会话、单条消息、岗位状态

**Resume Request（简历请求）**:
招聘者在 HR 会话中发出的、请求候选人提供简历的结构化卡片事件；它以平台来源消息标识唯一化，不能被普通文本消息替代。
_Avoid_: Resume、Resume Delivery、普通聊天消息

**Resume Delivery（简历投递）**:
某个已确认简历版本向指定 HR 会话执行的一次外部发送动作；必须关联简历版本、内容哈希、授权和平台回执。
_Avoid_: 简历上传、岗位已投递、发送按钮点击

**Delivery Receipt（投递回执）**:
用于证明外部平台动作结果的结构化证据，按传输确认、会话历史和平台业务状态分层；单一传输 ACK 不等于业务完成。
...
_Avoid_: 未溯源 JD、模型猜测、通用岗位描述

**Contact Channel（投递渠道）**:
招聘方公开提供的邮箱、官网表单、职位链接或账号；渠道必须记录发现来源和可信度，不能从公司名猜测邮箱。
