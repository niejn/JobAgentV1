...
TEXT = (Path(__file__).parents[1] / 'jobagent/ui/fixtures/job-card-single-line.txt').read_text(encoding='utf-8').strip()
SPACED_TEXT = (Path(__file__).parent / 'fixtures/job-card-spaced-ocr.txt').read_text(encoding='utf-8').strip()
...
@pytest.mark.parametrize('text', [TEXT, TEXT.replace(' ', '\n'), TEXT.replace('职位描述', '职 位 描 述').replace('企业级', '企 业 级')], ids=['single-line', 'multiline', 'ocr-spacing'])
...
def test_extract_endpoint_and_correction():
    with TestClient(web.app) as client:
        ...
        response = client.post('/api/journey-drafts/extract', json={'text': '公司：示例科技 岗位：后端开发 部门：研发部', 'current': payload['draft']})
        assert response.status_code == 200
...
def test_ocr_endpoint_returns_same_summary(monkeypatch, ocr_text):
    monkeypatch.setattr(web.TesseractOcrExtractor, 'extract', lambda self, path: SimpleNamespace(text=ocr_text, confidence=.8, engine='fixture', language='chi_sim'))
    with TestClient(web.app) as client:
        response = client.post('/api/journey-drafts/ocr', files={'file': ('jd.png', b'fixture-image', 'image/png')})
        assert response.status_code == 200
        ocr = response.json()
