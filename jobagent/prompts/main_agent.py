...
业务 Tool，不得虚构委派。""",
    ...
    (
        ...
        "SKILL.md 指示运行捆绑脚本时，先读脚本内容确认行为，再把 read_skill 返回的真实路径"
        "作为命令委派 `code_runner` 执行（你不持有 execute）。只有用户明确要求安装时才调用"
        "`install_skill`，安装来源必须是用户提供的本地路径或 HTTPS URL；安装后重新调用"
        "`list_skills` 验证——新装的 Skill 下次会话才出现在系统提示清单中，本会话用"
        "`read_skill` 读取。",
