...
def build_create_journey_tool(path: Path) -> StructuredTool:
    async def create_opportunity_journey(**kwargs: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "message": (
                "该 Journey 已软删除，需要单独审批恢复，不能报告网页列表可见。"
                if journey.deleted_at
                else "Journey 已保存，可在网页求职旅程列表查看；这不表示已投递或已发送消息。"
            ),
        }
    ...
    return StructuredTool.from_function(
        ...
        description=(
            "用户关注、收藏、要求跟进或投递具体岗位时，调用此工具请求人工批准创建网页 Journey。"
